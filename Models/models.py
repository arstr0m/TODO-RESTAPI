...
class Users(Base):
    __tablename__ = "users"
    id_user: Mapped[int] = mapped_column(Integer, primary_key=True, nullable=False)
    created_at = Column(TIMESTAMP(timezone=True), server_default=text('now()'))
    first_name = Column(String(20), nullable=False)
    last_name = Column(String(40), nullable=False)
    username = Column(String(30), nullable=False, index=True)
    email = Column(String(50), nullable=False)
    password = Column(String, nullable=False, server_default='FALSE')
    status = Column(String(3), nullable=False, server_default='ACT')

    tasks_users: Mapped[List["TasksUsers"]] = relationship("TasksUsers", back_populates="users")
...
class TasksUsers(Base):
    ...
    icons: Mapped["Icons"] = relationship("Icons", back_populates="tasks_users")
